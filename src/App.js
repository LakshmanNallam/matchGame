...
import './App.css'
import TabListItems from './Components/TabListItems/TabListtems'
import RenderListImage from './Components/renderListImges/RenderListImges'
...
class App extends Component {
  ...
  startTimer = () => {
    const {seconds, isGameRunning} = this.state
    console.log(seconds === 0 || isGameRunning)
    if (seconds === 1) {
      clearInterval(this.timerId)
      this.setState({isGameRunning: false, seconds: 0})
    } else {
  ...
  ImgClicked = id => {
    const {RandomImg, score} = this.state
    if (RandomImg.id === id) {
      this.setState({
        score: score + 1,
        RandomImg: imagesList[Math.floor(Math.random() * imagesList.length)],
  ...
  render() {
    const {currentTab, score, RandomImg, isGameRunning, seconds} = this.state
    ...
    return isGameRunning ? (
      ...
      <div className="minDiv">
        ...
        <div className="GameCon">
          <div className="bgConFOrGameOVERCON">
            <img
            ...
            <button
              className="llastBtn"
              type="button"
              onClick={this.btnClickedTorestarGame}
            >
              <img
